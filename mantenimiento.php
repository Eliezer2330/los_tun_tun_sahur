...
$ordenes = []; $kpi = ['pendiente'=>0,'en_proceso'=>0,'finalizado'=>0]; $dbError = null;
...
try {
    ...
    $ordenes = $pdo->query('
        SELECT m.id, m.tecnico_responsable, m.descripcion, m.estado,
               m.fecha_inicio, m.fecha_estimada_fin, m.costo,
               DATE_FORMAT(m.created_at,"%d/%m/%Y") AS creado,
               e.nombre AS espacio_nombre, e.edificio,
               i.tipo AS incid_tipo, i.prioridad AS incid_prioridad
        FROM mantenimiento m
        JOIN espacios e ON e.id = m.espacio_id
        LEFT JOIN incidencias i ON i.id = m.incidencia_id
        ORDER BY FIELD(m.estado,"en_proceso","pendiente","finalizado"), m.fecha_inicio DESC
    ')->fetchAll();
    foreach ($ordenes as $o) { if (isset($kpi[$o['estado']])) $kpi[$o['estado']]++; }
...
<title>SIGMEA — Mantenimiento</title>
...
.badge.pendiente{background:var(--yellow-bg);color:var(--yellow);border:1px solid var(--yellow-bd)}
.badge.en_proceso{background:rgba(79,142,247,.1);color:var(--accent);border:1px solid rgba(79,142,247,.25)}
.badge.finalizado{background:var(--green-bg);color:var(--green);border:1px solid var(--green-bd)}
.desc-cell{max-width:240px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:var(--muted);font-size:12px}
...
<aside class="sidebar">
    ...
    <nav class="nav-section">
        ...
        <a class="nav-item active" href="mantenimiento.php"><span class="ico">🔧</span> Mantenimiento</a>
...
<main class="main">
    <div class="topbar">
        <div>
            ...
            <p class="page-sub">Órdenes de mantenimiento y reparaciones</p>
        </div>
    </div>
    ...
    <div class="kpi-row">
        <div class="kpi yellow"><div class="kpi-val"><?= $kpi['pendiente'] ?></div><div class="kpi-lbl">Pendientes</div><div class="kpi-icon">⏳</div></div>
        <div class="kpi red"><div class="kpi-val"><?= $kpi['en_proceso'] ?></div><div class="kpi-lbl">En proceso</div><div class="kpi-icon">🔧</div></div>
        <div class="kpi green"><div class="kpi-val"><?= $kpi['finalizado'] ?></div><div class="kpi-lbl">Finalizados</div><div class="kpi-icon">✅</div></div>
    </div>

    <div class="table-wrap">
        <div class="table-header">
            <span class="table-title">Órdenes de Mantenimiento</span>
        ...
        <table id="mainTable">
            <thead>
                <tr><th>#</th><th>Espacio</th><th>Técnico</th><th>Descripción</th><th>Estado</th><th>Inicio</th><th>Fin estimado</th><th>Costo</th></tr>
            </thead>
            <tbody>
            <?php if (empty($ordenes)): ?>
            <tr><td colspan="8"><div class="empty-state"><div class="ico">✅</div><p>No hay órdenes de mantenimiento.</p></div></td></tr>
            <?php else: ?>
            <?php foreach ($ordenes as $o): ?>
            <tr>
                <td style="color:var(--muted)">#<?= $o['id'] ?></td>
                <td><strong><?= htmlspecialchars($o['espacio_nombre']) ?></strong> <span style="color:var(--muted);font-size:11px"><?= $o['edificio'] ?></span></td>
                <td><?= htmlspecialchars($o['tecnico_responsable']) ?></td>
                <td class="desc-cell" title="<?= htmlspecialchars($o['descripcion']) ?>"><?= htmlspecialchars($o['descripcion']) ?></td>
                <td><span class="badge <?= $o['estado'] ?>"><?= ucfirst(str_replace('_',' ',$o['estado'])) ?></span></td>
                <td style="font-size:12px"><?= $o['fecha_inicio'] ?></td>
                <td style="font-size:12px;color:var(--muted)"><?= $o['fecha_estimada_fin'] ?? '—' ?></td>
                <td style="font-size:12px"><?= $o['costo'] ? '$'.number_format($o['costo'],2) : '<span style="color:var(--muted)">—</span>' ?></td>
            </tr>
            <?php endforeach; ?>
            <?php endif; ?>
            </tbody>
